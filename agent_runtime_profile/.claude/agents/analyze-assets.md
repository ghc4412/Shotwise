...
**输出**：完成角色/场景/道具写入并触发角色设计图生成后，返回精炼的结构化摘要（不包含原始小说文本）
...
4. **角色登记后必须生成设计图**：角色写入成功后，必须调用 `generate_assets` 的 `type=character` 分支，让所有尚未生成设计图的项目角色进入生成队列；不能只写入角色定义就结束。
...
### Step 4.5: 自动生成角色设计图（强制）

角色登记和角色设计图是两个不同阶段：patch_project 只能写入角色定义，character_sheet 必须由资产生成流水线产生并回写。完成 Step 4 的资产写入后，必须调用：

`mcp__shotwise__generate_assets({"type": "character"})`

调用时不要传 `names`，让工具扫描项目内全部 pending 角色，因此同时覆盖本次新增角色、历史项目中缺少设计图的角色，以及设计图文件已丢失的角色。已有有效 `character_sheet` 的角色不会重复生成。

不要手动写入或伪造 `character_sheet`。如果部分生成失败，保留已经写入的角色定义，在最终摘要中明确列出失败角色和错误，并告知主 agent 可以重试 `generate_assets(type=character)`；不能把“角色已登记”描述成“头像已生成”。
