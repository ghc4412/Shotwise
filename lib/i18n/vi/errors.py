MESSAGES = {
    ...
    "episode_duration_revision_conflict": "Kế hoạch thời lượng của tập đã thay đổi, hãy làm mới rồi thử lại",
    "episode_duration_script_rebound": "Liên kết kịch bản của tập đã thay đổi, hãy làm mới rồi thử lại",
    "episode_duration_resource_not_found": "Không tìm thấy tập hoặc cảnh quay mục tiêu",
    "episode_duration_invalid": "Kế hoạch thời lượng của tập không hợp lệ: {details}",
    ...
    "generation_asset_preflight_failed": "Kiểm tra trước khi tạo thất bại. Tài nguyên chưa đăng ký: {unregistered}; ảnh tài nguyên bị thiếu: {missing_images}; biến thể nhân vật bị thiếu: {missing_variants}",
    ...
    "character_variant_not_found": "Biến thể nhân vật '{name}' không tồn tại",
    "character_variant_slug_exists": "Slug biến thể nhân vật '{slug}' đã tồn tại",
    "character_variant_invalid_slug": "Slug biến thể nhân vật không hợp lệ",
    "character_variant_invalid_image_path": "Đường dẫn ảnh biến thể nhân vật không hợp lệ",
    "character_variant_invalid_status": "Trạng thái biến thể nhân vật không hợp lệ",
    "character_avatar_file_empty": "Tệp ảnh đại diện không được để trống",
    "character_avatar_too_large": "Tệp ảnh đại diện không được vượt quá {max_mb} MB",
    "character_avatar_invalid_file": "Tệp ảnh đại diện không phải là ảnh hợp lệ",
    "character_avatar_pixels_too_large": "Ảnh đại diện có quá nhiều điểm ảnh; vui lòng giữ dưới {max_megapixels} megapixel",
    "character_task_submitted": "Đã gửi tác vụ tạo thiết kế nhân vật cho '{name}'",
    "character_relations_revision_conflict": "Quan hệ nhân vật đã được thao tác khác cập nhật. Hãy làm mới rồi thử lại.",
    "character_relations_analysis_invalid": "AI trả về quan hệ nhân vật không hợp lệ: {message}",
    "character_relations_analysis_failed": "Phân tích quan hệ bằng AI thất bại. Hãy thử lại sau hoặc đổi mô hình văn bản.",
    ...
    "script_review_quarantine_schema_unsupported": (
        "Bản nháp bị cách ly sử dụng phiên bản schema {version} không được hỗ trợ; hãy nâng cấp ứng dụng trước khi tiếp tục"
    ),
    ...
    "video_resolution_not_supported": "Mô hình {model} không hỗ trợ độ phân giải {resolution}; hãy dùng {supported}",
    ...
    "video_kling_model_endpoint_unsupported": "Endpoint API Kling hiện tại không hỗ trợ tạo video từ nhiều ảnh tham chiếu cho model {model} ({endpoint}); hãy dùng base URL hoặc model hỗ trợ, hoặc bỏ ảnh tham chiếu và dùng ảnh đầu tiên hoặc tạo video từ văn bản",
    ...
    "assistant_images_too_large": "Tổng kích thước tệp đính kèm trò chuyện vượt quá giới hạn; hãy xóa hoặc nén ảnh rồi thử lại",
    "assistant_image_pixels_too_large": "Độ phân giải tệp đính kèm trò chuyện quá cao; hãy thu nhỏ ảnh rồi thử lại",
    "assistant_image_type_unsupported": "Loại tệp đính kèm trò chuyện này không được hỗ trợ; hãy tải lên tệp ảnh",
    "asset_generation_reused": "Tài sản “{name}” đã có ảnh thiết kế có thể sử dụng; đã bỏ qua việc tạo trùng",
    ...
    "memory_scope_invalid": "Phạm vi bộ nhớ không hợp lệ, chỉ chấp nhận user hoặc project",
    "memory_project_required": "Bộ nhớ theo dự án phải có dự án",
    "memory_project_invalid": "Định danh dự án của bộ nhớ không hợp lệ",
    "memory_category_invalid": "Danh mục bộ nhớ không hợp lệ",
    "memory_source_invalid": "Nguồn bộ nhớ không hợp lệ",
    "memory_content_invalid": "Nội dung bộ nhớ không hợp lệ",
    "memory_content_empty": "Nội dung bộ nhớ không được để trống",
    "memory_content_too_long": "Nội dung bộ nhớ quá dài, tối đa {max_length} ký tự",
    "memory_sensitive_content": "Nội dung bộ nhớ có vẻ chứa khóa, mật khẩu hoặc đường dẫn nhạy cảm nên không thể lưu",
    "memory_prompt_injection": "Nội dung bộ nhớ có vẻ chứa lệnh chèn prompt nên không thể lưu",
    "memory_duplicate": "Bộ nhớ giống hệt đã tồn tại",
    "memory_update_empty": "Hãy cung cấp ít nhất một trường cần cập nhật",
    "memory_not_found": "Không tìm thấy bộ nhớ",
    "memory_candidate_not_found": "Không tìm thấy ứng viên bộ nhớ",
