MESSAGES = {
    ...
    "workflow_template_not_published": "该模板尚未发布，暂时无法使用",
    "workflow_version_conflict": "工作流已变化：预期版本 {expected}，当前版本 {actual}",
    "project_not_found": "项目 '{name}' 不存在或未初始化",
    "resource_not_found": "请求的资源不存在",
    "episode_duration_revision_conflict": "分集时长配置已变化，请刷新后重试",
    "episode_duration_script_rebound": "该集剧本绑定已变化，请刷新后重试",
    "episode_duration_resource_not_found": "目标分集或镜头不存在",
    "episode_duration_invalid": "分集时长配置无效：{details}",
    ...
    "forbidden": "权限不足：该操作需要管理员权限",
    "generation_asset_preflight_failed": "生成前检查失败。未登记资产：{unregistered}；缺少资产图：{missing_images}；缺少角色衍生形态：{missing_variants}",
    ...
    "character_variant_not_found": "角色衍生形态「{name}」不存在",
    "character_variant_slug_exists": "角色衍生形态标识「{slug}」已存在",
    "character_variant_invalid_slug": "角色衍生形态标识无效",
    "character_variant_invalid_image_path": "角色衍生形态图片路径无效",
    "character_variant_invalid_status": "角色衍生形态状态无效",
    "character_avatar_file_empty": "头像文件不能为空",
    "character_avatar_too_large": "头像文件不能超过 {max_mb} MB",
    "character_avatar_invalid_file": "头像文件不是有效图片",
    "character_avatar_pixels_too_large": "头像图片像素过多，请控制在 {max_megapixels} 百万像素以内",
    "character_task_submitted": "角色「{name}」设计图生成任务已提交",
    "character_relations_revision_conflict": "角色关系已被其他操作更新，请刷新后重试",
    "character_relations_analysis_invalid": "AI 返回的角色关系无效：{message}",
    "character_relations_analysis_failed": "AI 角色关系分析失败，请稍后重试或更换文本模型",
    ...
    "script_review_quarantine_schema_unsupported": "隔离草稿使用了不支持的 schema 版本 {version}，请先升级应用后再继续",
    "draft_event_label": "第 {episode} 集{label_prefix}",
    "normalized_script": "规范化剧本",
    "segment_splitting": "片段拆分",
    # Source loader
    "source_unsupported_format": "不支持的文稿格式：{ext}（支持 .txt / .md / .docx / .epub / .pdf）",
    ...
    "source_conflict": "文稿「{existing}」已存在，建议改名为「{suggested}」",
    # Providers
    ...
    "source_kind_not_editable": "项目创建后不支持修改文稿性质（source_kind）",
    ...
    "video_resolution_not_supported": "模型 {model} 不支持 {resolution} 分辨率；请使用 {supported}",
    ...
    "video_kling_model_endpoint_unsupported": "Kling 当前 API 地址不支持模型 {model} 的多图主体参考生视频（{endpoint}）；请更换支持该模型的 base URL 或模型，或移除参考图后使用首帧/文生视频",
    ...
    "assistant_image_type_unsupported": "聊天附图格式不受支持，请上传图片文件",
    "asset_generation_reused": "资产“{name}”已有可用设计图，已跳过重复生成",
    "workflow_template_upgrade_confirmation_required": "模板升级需要确认",
    "workflow_template_upgrade_unavailable": "没有可用的已发布模板升级",
    "workflow_template_upgrade_incompatible": "模板升级不兼容：{reasons}",
    "workflow_quality_gate_required": "不能绕过质量门禁",
    "memory_scope_invalid": "记忆作用域无效，只能是 user 或 project",
    "memory_project_required": "项目记忆必须指定项目",
    "memory_project_invalid": "项目记忆的项目标识无效",
    "memory_category_invalid": "记忆分类无效",
    "memory_source_invalid": "记忆来源无效",
    "memory_content_invalid": "记忆内容无效",
    "memory_content_empty": "记忆内容不能为空",
    "memory_content_too_long": "记忆内容过长，最多 {max_length} 个字符",
    "memory_sensitive_content": "记忆内容疑似包含密钥、密码或敏感路径，无法保存",
    "memory_prompt_injection": "记忆内容疑似包含提示注入指令，无法保存",
    "memory_duplicate": "相同记忆已存在",
    "memory_update_empty": "至少提供一个需要更新的字段",
    "memory_not_found": "记忆不存在",
    "memory_candidate_not_found": "记忆候选不存在",
