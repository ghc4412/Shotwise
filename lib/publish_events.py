...
def emit_publish_job_events(
    ...
    *,
    ...
    """Emit publish-job refresh signals without failing a state update.

    The database remains the source of truth; consumers use this event only
    to re-fetch the affected publish jobs. Event delivery is best effort so a
    missing or shutting-down SSE service cannot fail a publish transition.
    """
    ...
    try:
        ...
        logger.exception(
            "发送发布任务项目事件失败 project=%s count=%d",
            normalized_project,
            len(changes),
        )
