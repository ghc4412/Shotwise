...
export function ReferenceVideoCanvas({
  ...
  // 批量生成的作用对象是待生成或上次失败的 unit。失败任务是可重试的终态，不能
  // 因为 statusMap 为 failed 就把批量入口误判为没有可执行对象。
  const batchTargets = useMemo(
    () => units.filter((u) => statusMap[u.unit_id] === "pending" || statusMap[u.unit_id] === "failed"),
    [units, statusMap],
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div
        ...
        className="flex items-center gap-0.5 border-b border-[var(--color-hairline)] bg-[var(--color-shell-side-a)] px-5"
      >
        <button
          type="button"
          role="tab"
        ...
        {tab === "units" && (
          <button
            ...
            className="focus-ring inline-flex items-center gap-1.5 rounded-md border border-[var(--color-hairline)] bg-[var(--color-shell-btn)] px-2.5 py-1 text-[11.5px] text-[var(--color-text-2)] transition-colors hover:bg-[var(--color-shell-hover-strong)] hover:text-[var(--color-text)] disabled:cursor-not-allowed disabled:opacity-60"
          >
      ...
      {tab === "preproc" ? (
        <div className="min-h-0 flex-1 overflow-auto bg-[var(--color-bg-grad-a)]">
        ...
        <div
          ref={workbenchRef}
          className="relative min-h-0 flex-1 overflow-hidden bg-[var(--color-bg-grad-a)]"
        >
          <div className="grid h-full min-h-0" style={{ gridTemplateColumns: gridCols }}>
            ...
            <div className="flex min-h-0 flex-col overflow-hidden bg-[var(--panel-card-bg)]">
              {selected ? (
                <>
                  <div className="flex flex-wrap items-center gap-2 border-b border-[var(--color-hairline-soft)] px-4 py-2.5">
                    ...
                    <button
                      ...
                      className="focus-ring inline-grid h-6 w-6 place-items-center rounded border border-[var(--color-hairline)] bg-[var(--color-shell-btn)] text-[var(--color-text-2)] hover:bg-[var(--color-shell-hover-strong)] disabled:cursor-not-allowed disabled:opacity-50"
                    >
                  ...
                  {stackPreview && (
                    <div
                      ...
                      className="flex items-center gap-0 border-b border-[var(--color-hairline)] bg-[var(--color-shell-side-a)] px-5"
                    >
                      <button
                        type="button"
                        role="tab"
                  ...
                  <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
                    {(!stackPreview || stackTab === "editor") && (
                      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
                            ...
                            <button
                              ...
                              className={`focus-ring rounded-md border px-2.5 py-1 text-[11.5px] font-medium transition-colors ${
                                editorView === view
                                  ...
                                  : "border-[var(--color-hairline)] bg-[var(--color-shell-btn)] text-[var(--color-text-3)] hover:text-[var(--color-text-2)]"
                              }`}
                            >
                              {view === "script"
                        ...
                        {/* Editor bottom bar */}
                        <div className="flex flex-shrink-0 items-center gap-2 border-t border-[var(--color-hairline-soft)] bg-[var(--color-shell-footer)] px-3.5 py-2">
                          <span
                          ...
                          <button
                            ...
                            className={`focus-ring inline-flex min-w-[80px] items-center justify-center gap-1.5 rounded-md px-3 py-1 text-xs font-semibold ${
                              isDirty
                                ...
                                : "border border-[var(--color-hairline)] bg-[var(--color-shell-btn)] text-[var(--color-text-4)]"
                    ...
                    {stackPreview && stackTab === "preview" && (
                      <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--color-shell-panel)]">
                        <UnitPreviewPanel
                          unit={selected}
                          projectName={projectName}
            ...
            {!stackPreview && (
              <div className="flex min-h-0 flex-col overflow-hidden border-l border-[var(--color-hairline)] bg-[var(--color-shell-panel)]">
                <UnitPreviewPanel
                  unit={selected}
                  projectName={projectName}
