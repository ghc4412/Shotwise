...
export function UnitRail({ units, selectedId, onSelect, onExpand, dirtyMap, statusMap }: UnitRailProps) {
  ...
  return (
    <div className="flex h-full min-h-0 w-[56px] flex-col overflow-hidden border-r border-[var(--color-hairline)] bg-[var(--color-shell-side-a)]">
      <button
        type="button"
        onClick={onExpand}
        title={t("reference_unit_rail_expand")}
        aria-label={t("reference_unit_rail_expand")}
        className="focus-ring mx-auto mb-1.5 mt-2.5 grid h-[30px] w-[34px] place-items-center rounded border border-[var(--color-hairline-soft)] bg-[var(--color-shell-btn)] text-[var(--color-text-3)] hover:text-[var(--color-text)]"
      >
