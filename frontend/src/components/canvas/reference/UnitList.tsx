...
export function UnitList({ units, selectedId, onSelect, onAdd, dirtyMap, statusMap }: UnitListProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden border-r border-[var(--color-hairline)] bg-[var(--color-shell-side-a)]">
      <div className="flex items-center gap-2 px-3 pt-3 pb-2">
        ...
        <button
          type="button"
          onClick={onAdd}
          className="focus-ring inline-flex items-center gap-1 rounded border border-[var(--color-hairline-soft)] bg-[var(--color-shell-btn)] px-2 py-0.5 text-[11px] text-[var(--color-text-3)] hover:text-[var(--color-text)]"
        >
      ...
      <div className="px-3 pb-2">
        <label className="flex items-center gap-1.5 rounded-md border border-[var(--color-hairline-soft)] bg-[var(--color-shell-field)] px-2 py-1.5">
            ...
            return (
              <li
                ...
                className={`focus-ring relative mb-1 cursor-pointer rounded-lg p-2.5 text-sm transition-colors ${
                  selected
                    ? "border border-[var(--color-accent-soft)] bg-[var(--color-accent-dim)]"
                    : "border border-transparent hover:bg-[var(--color-shell-hover)]"
                }`}
              >
                {selected && (
                  <span
                  ...
                  <span
                    className={`rounded px-1.5 py-0.5 font-mono text-[11px] font-bold tracking-wider ${
                      selected
                        ...
                        : "bg-[var(--color-shell-badge)] text-[var(--color-text-3)]"
                    }`}
                    translate="no"
                  >
                    {u.unit_id}
