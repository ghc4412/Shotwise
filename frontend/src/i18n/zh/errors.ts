...
export default {
  ...
  'source_unsupported_format': '不支持的文稿格式：{{ext}}',
  'source_decode_failed': '文稿「{{filename}}」解码失败（已尝试：{{tried}}）',
  'source_corrupt_file': '文稿「{{filename}}」无法解析：{{reason}}',
  'source_too_large': '文稿「{{filename}}」过大（{{size_mb}} MB > {{limit_mb}} MB）',
  'source_conflict': '文稿「{{existing}}」已存在',
  // Image Capability
