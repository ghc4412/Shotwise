"""Cross-check that every backend Shotwise MCP tool has a frontend display name.

The single source of truth is :data:`SHOTWISE_MCP_TOOL_IDS` in
``server/agent_runtime/sdk_tools/__init__.py``. The frontend renders each
``mcp__shotwise__<id>`` tool chip by looking up ``tool_name_<id>`` in the
...
def test_every_backend_tool_has_frontend_display_name(locale: str) -> None:
    ...
    expected = {f"tool_name_{tid}" for tid in SHOTWISE_MCP_TOOL_IDS}
    missing = expected - keys
    assert not missing, (
        f"frontend/src/i18n/{locale}/dashboard.ts 缺少 MCP tool 显示名翻译: {sorted(missing)}。"
        f" 单一真相源在 server/agent_runtime/sdk_tools/__init__.py 的 SHOTWISE_MCP_TOOL_IDS。"
    )
...
def test_no_orphan_tool_name_keys_in_any_locale() -> None:
    ...
    for locale in LOCALES:
        ...
        assert not orphans, (
            ...
            f"{sorted(orphans)}。请删除或更新 backend SHOTWISE_MCP_TOOL_IDS。"
        )
