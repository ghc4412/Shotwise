...
def test_collect_returns_text(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert "Shotwise diagnostics" in text
...
def test_collect_masks_db_password(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert db_line == "Database URL: postgresql+asyncpg://••••:••@db.example.com:5432/shotwise"
...
def test_collect_masks_db_query_secrets(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv(
        "DATABASE_URL",
        "postgresql://host.example.com/shotwise?sslmode=require&password=topsecret&token=abc123",
    )
    _reset_for_tests()
    ...
    assert (
        db_line
        == "Database URL: postgresql://host.example.com/shotwise?sslmode=require&password=%E2%80%A2%E2%80%A2&token=%E2%80%A2%E2%80%A2"
    )
