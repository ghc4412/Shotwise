...
- **导出范围选项**：在导出交互中新增选择——"导出全部" 与 "仅当前版本"：
  ...
  - **仅当前版本**：跳过 `versions/` 目录下的历史文件，仅保留当前使用的资源。同时在 `SHOTWISE-export.json` 清单中记录 `scope: "current"` 标记，并在 `versions.json` 中仅保留 current version 条目作为元数据（保留 prompt 等生成信息），以便导入时恢复上下文。
