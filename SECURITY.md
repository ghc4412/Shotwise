...
Shotwise provides security fixes for the latest release and the current `main` branch. Older releases do not receive guaranteed backports; users should upgrade before requesting a security fix.

## Supported deployment boundary

Shotwise currently supports:
...
Direct Internet exposure is not currently supported. Instances shared by mutually untrusted users are also unsupported because Shotwise does not provide tenant isolation, role-based access control, or per-user project authorization.
...
Report suspected vulnerabilities through [GitHub Private Vulnerability Reporting](https://github.com/Shotwise/Shotwise/security/advisories/new). Include the affected version or commit, prerequisites, impact, and a minimal reproducer when possible.
