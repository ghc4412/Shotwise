...
async def delete_item(
    board_id: str, item_id: str, user: CurrentUser, session: AsyncSession = Depends(get_async_session)
):
    try:
        return await service.delete_item(session, board_id, item_id, user_id=user.id)
    except service.CreativeBoardNotFoundError as exc:
        raise _not_found() from exc
...
async def delete_board(board_id: str, user: CurrentUser, session: AsyncSession = Depends(get_async_session)):
    try:
        return await service.delete_board(session, board_id, user_id=user.id)
    except service.CreativeBoardNotFoundError as exc:
        raise _not_found() from exc
...
async def delete_edge(
    ...
    try:
        return await service.delete_edge(session, board_id, edge_id, user_id=user.id)
    except service.CreativeBoardNotFoundError as exc:
        raise _not_found() from exc
