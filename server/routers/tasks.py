...
from lib.task_failure import render_failure, sanitize_failure_reason
...
def _localize_task(task: dict[str, Any], translate: Callable[..., str]) -> dict[str, Any]:
    ...
    Known structured codes become localized text. Raw exception text and legacy rows
    are sanitized before rendering so historical database rows cannot leak provider
    credentials. Generation warnings stored as ``result.warnings`` (``{key, params}``
    entries written by the reference-video pipeline) are rendered in place into a list
    of strings, mirroring how ``error_message`` is rendered. The input dict is never
    mutated — a rendered copy is returned — so dicts owned by the queue layer stay
    locale-neutral and cannot be polluted across requests.
    """
    localized = task
    message = localized.get("error_message")
    if message:
        safe_message = sanitize_failure_reason(message)
        localized = {**localized, "error_message": render_failure(safe_message, translate)}
