...
- **未来 plugin 机制必须延续这条约束**：当运行时 plugin 机制（参考 `docs/research/shotwise-video-api-protocol-research.md` §7.5）落地时，plugin endpoint 同样只能接 `api_key` + `base_url`；plugin 想暴露多字段协议须自带"内置 provider 风格"声明（且该路径等同于 plugin 自己提供 PROVIDER_REGISTRY 扩展，复杂度显著上升，本 ADR 范围不预设）。这条约束让 plugin 接口面保持稳定。
