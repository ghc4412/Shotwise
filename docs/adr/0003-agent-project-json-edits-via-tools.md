...
工具集（均为 in-process MCP `shotwise`，跑在 server 进程、不在 agent sandbox 内）：
...
- 工具**返回文本**是 agent-facing（免 i18n）；工具**显示名**是 user-facing，须在 `SHOTWISE_MCP_TOOL_IDS` 注册并补 `tool_name_<id>` 三语（zh/en/vi）。
