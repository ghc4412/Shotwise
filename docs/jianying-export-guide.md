...
将 SHOTWISE 已生成的视频片段按集导出为剪映（JianYing）草稿文件，在剪映桌面版中直接打开并进行二次编辑——调整节奏、添加字幕、转场、配音等。

## 前置条件

- 已在 SHOTWISE 中完成至少一集的视频片段生成
- 本地已安装 **剪映桌面版**（5.x 或 6+）
...
导出仅包含已成功生成的视频片段。如果某些片段尚未生成或生成失败，它们不会出现在草稿中。回到 SHOTWISE 补充生成后重新导出即可。
