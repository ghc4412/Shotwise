...
**Goal:** 将 Shotwise 单集已生成的视频片段导出为剪映草稿 ZIP，用户解压到本地剪映草稿目录后直接在剪映中打开编辑。
...
- **草稿目录输入**：`<input type="text">`，`onChange` 时同步写 `localStorage.setItem("shotwise_jianying_draft_path", value)`
