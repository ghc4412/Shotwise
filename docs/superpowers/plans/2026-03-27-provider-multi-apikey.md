...
运行: `uv run python -c "import sqlite3; conn = sqlite3.connect('projects/.SHOTWISE.db'); print([r[1] for r in conn.execute('PRAGMA table_info(provider_credential)').fetchall()])"`
